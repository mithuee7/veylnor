...
async function renderSegment(c: RenderClip, out: string, width: number, height: number, onProgress?: ProgressFn) {
  ...
  await run([
    ...
    "-vf", `scale=${width}:${height}:force_original_aspect_ratio=increase:flags=fast_bilinear,crop=${width}:${height},setsar=1,fps=30,format=yuv420p`,
...
function buildTextFilter(textFile: string, textOverlay?: TextOverlay) {
  ...
  if (textOverlay.font === "EB Garamond") {
    ...
    return `drawtext=fontfile='${fontFile}':textfile='${textfile}':fontcolor=white:fontsize=${size}:x=(w-text_w)/2:y=(h-text_h)/2:shadowcolor=black@0.75:shadowx=3:shadowy=3:borderw=1:bordercolor=black@0.22:fix_bounds=1`;
  }
  ...
  return `drawtext=font='${font}':textfile='${textfile}':fontcolor=white:fontsize=${size}:x=(w-text_w)/2:y=(h-text_h)/2:shadowcolor=black@0.75:shadowx=3:shadowy=3:borderw=1:bordercolor=black@0.22:fix_bounds=1`;
}
