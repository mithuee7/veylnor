...
export async function waitForRenderJob(id: string, timeoutMs = 9 * 60 * 1000) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const job = getRenderJob(id);
    if (!job) throw new Error("Render job disappeared before completion.");
    if (job.status === "complete") return job;
    if (job.status === "failed") throw new Error(job.error ?? "FFmpeg render failed.");
    await new Promise(resolve => setTimeout(resolve, 150));
  }
  throw new Error("Render did not finish before the API request timeout.");
}
