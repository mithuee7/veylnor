...
export default function SequenceEditor({
  ...
  function replaceClip(nextClip: SequenceItem["clip"]) {
    if (!replacing) return;
    const targetDuration = Number(replacing.targetDuration);
    const sourceDuration = Number(nextClip.duration ?? 0);
    if (sourceDuration > 0 && sourceDuration + 0.01 < targetDuration) return;
    onChange(items.map((it) => it.position === replacing.position ? {
      ...it,
      category: nextClip.category,
      clip: nextClip,
      // Manual replacement always starts at the beginning, but keeps the
      // original scene's exact slot duration and position.
      sourceStart: 0,
      sourceEnd: Number(targetDuration.toFixed(3)),
      targetDuration,
    } : it));
    setReplacing(null);
  }
