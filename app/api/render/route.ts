...
interface Body {
  ...
  songStart: number;
  songEnd: number;
  sequence: { clipId: string; start: number; end: number; targetDuration?: number; hold?: boolean }[];
  ...
  try {
    ...
    for (const item of body.sequence) {
      ...
      if (item.targetDuration !== undefined) {
        ...
        // A single cut-free shot shorter than its slot is allowed only when flagged: the renderer holds its last frame.
        const okSpan = item.hold === true ? sourceSpan >= 0.1 && sourceSpan <= target + 0.05 : Math.abs(sourceSpan - target) <= 0.05;
        if (!(target > 0) || !okSpan) {
